...
public class BoardController {
	...
	public String view(HttpServletRequest request, Model model) {
		...
		if(request.getParameter("searchFlag") != null)	{
			sseq = request.getParameter("sseq");
			ScategoryTO sto = new ScategoryTO();
			sto.setSeq(sseq);
			ArrayList<ScategoryTO> searchSC = mapper.searchSC(sseq);
			String lseq = "";
			for(ScategoryTO to : searchSC)	{
				lseq = to.getLseq();
				sname = to.getName();
			}
			lname = mapper.searchLname(lseq);
			model.addAttribute("sname",sname);
			model.addAttribute("lname",lname);
